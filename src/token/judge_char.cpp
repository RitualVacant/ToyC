...
extern char const useless_char[] = {'\n', '\r', '\t', ' '};

//测试词法划分是否正确的输出转换
extern std::unordered_map<token, std::string> const token_to_string{
  {token::constant_float_number,            "constant_float_number "          },
  {token::constant_negative_float_number,   "constant_negative_float_number"  },
  {token::constant_integer_number,          "constant_integer_number"         },
  {token::constant_negative_integer_number, "constant_negative_integer_number"},
  {token::constant_string,                  "constant_string"                 },
...
//二元运算符
bool is_binary_operator(token operator_) {
  if (operator_ == token::bit_and || operator_ == token::bit_not || operator_ == token::bit_or || operator_ == token::log_and || operator_ == token::log_not || operator_ == token::log_or || operator_ == token::equ || operator_ == token::not_equ || operator_ == token::great || operator_ == token::great_equ || operator_ == token::less || operator_ == token::less_equ || operator_ == token::plus || operator_ == token::minus || operator_ == token::times || operator_ == token::div || operator_ == token::mod)
    return true;
  else
    return false;
}
//公共运算符
bool is_primary_operator(token token_) {
  if (token_ == token::l_par || token_ == token::r_par || token_ == token::l_mid_par || token_ == token::r_mid_par || token_ == token::class_int || token_ == token::class_char || token_ == token::class_double || token_ == token::class_bool)
    return true;
  return false;
}
bool is_basic_type(token token_) {
  switch (token_) {
