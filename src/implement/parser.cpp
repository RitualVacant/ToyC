...
void parser::parser_if_statement() {
  ...
  std::string label_if = get_label();
  //
  switch (symbol) {
    case token::log_and:  //&&
      code.push_back(statement(token::log_and, value1, value2, label_if));
      break;
    case token::log_or:  //||
      code.push_back(statement(token::log_or, value1, value2, label_if));
      break;
    case token::equ:  //==
      code.push_back(statement(token::equ, value1, value2, label_if));
      break;
    case token::not_equ:  //!=
      code.push_back(statement(token::not_equ, value1, value2, label_if));
      break;
    case token::great_equ:  //>=
      code.push_back(statement(token::great_equ, value1, value2, label_if));
      break;
    case token::less_equ:  //<=
      code.push_back(statement(token::less_equ, value1, value2, label_if));
      break;
    case token::great:  //>
      code.push_back(statement(token::great, value1, value2, label_if));
      break;
    case token::less:  //<
      code.push_back(statement(token::less, value1, value2, label_if));
      break;
    default: break;
  }
  ...
  code.push_back(statement(token::label, label_if, "", ""));
...
void parser::parser_while_statement() {
  ...
  scan.next_token();  // (
  std::string label_while_judge = get_label();
  std::string label_while_end   = get_label();
  std::string label_while_block = get_label();
  code.push_back(statement(token::label, label_while_judge, "", ""));
  ...
  switch (symbol) {
    case token::log_and:  //&&
      code.push_back(statement(token::equ, value1, "0", label_while_end));
      code.push_back(statement(token::equ, value2, "0", label_while_end));
      break;
    case token::log_or:  //||
      code.push_back(statement(token::not_equ, value1, "0", label_while_block));
      code.push_back(statement(token::not_equ, value2, "0", label_while_block));
      break;
    case token::equ:  //==
      code.push_back(statement(token::equ, value1, value2, label_while_block));
      break;
    case token::not_equ:  //!=
      code.push_back(
          statement(token::not_equ, value1, value2, label_while_block));
      break;
    case token::great_equ:  //>=
      code.push_back(
          statement(token::great_equ, value1, value2, label_while_block));
      break;
    case token::less_equ:  //<=
      code.push_back(
          statement(token::less_equ, value1, value2, label_while_block));
      break;
    case token::great:  //>
      code.push_back(
          statement(token::great, value1, value2, label_while_block));
      break;
    case token::less:  //<
      code.push_back(statement(token::less, value1, value2, label_while_block));
      break;
    default:
  ...
  scan.next_token();
  code.push_back(statement(token::label, label_while_block, "", ""));

  while (scan.get_current_token() != token::r_big_par) {
    parser_statement();
  }

  code.push_back(statement(token::jmp, label_while_judge, "", ""));
  code.push_back(statement(token::label, label_while_end, "", ""));
  return;
}

// return

std::string parser::parser_return_statement() {
  scan.next_token();
...
* /
    ...
    token::end,       token::r_mid_par, token::r_par, token::log_and,
    token::log_not,   token::log_or,    token::equ,   token::not_equ,
    token::great_equ, token::less_equ,  token::great, token::less};

//转换成后缀表达式
std::vector<std::tuple<token, std::string>> parser::parser_pre_to_pos() {
  ...
  while (end_expresion.find(scan.get_current_token()) == end_expresion.end()) {
    ...
    switch (scan.get_current_token()) {
      ...
      case token::r_bool:    // bool
        res.push_back({scan.get_current_token(),
                       fmt::format("${}", scan.get_current_value())});
        break;
      ...
      case token::plus:  //+
        if (symbol_stack.empty() ||
            preceden(token::plus) > preceden(std::get<0>(symbol_stack.back())))
          symbol_stack.push_back(scan.now_token);
        else {
          while (preceden(token::plus) <=
                 preceden(std::get<0>(symbol_stack.back()))) {
      ...
      case token::minus:  //-
        if (symbol_stack.empty() ||
            preceden(token::minus) > preceden(std::get<0>(symbol_stack.back())))
          symbol_stack.push_back(scan.now_token);
        else {
          while (preceden(token::minus) <=
                 preceden(std::get<0>(symbol_stack.back()))) {
            res.push_back(symbol_stack.back());
            symbol_stack.pop_back();
          }
      ...
      case token::times:
        if (symbol_stack.empty() ||
            preceden(token::times) > preceden(std::get<0>(symbol_stack.back())))
          symbol_stack.push_back(scan.now_token);
        else {
          while (preceden(token::times) <=
                 preceden(std::get<0>(symbol_stack.back()))) {
            res.push_back(symbol_stack.back());
            symbol_stack.pop_back();
          }
      ...
      case token::div:  ///
        if (symbol_stack.empty() ||
            preceden(token::div) > preceden(std::get<0>(symbol_stack.back())))
          symbol_stack.push_back(scan.now_token);
        else {
          while (preceden(token::div) <=
                 preceden(std::get<0>(symbol_stack.back()))) {
            res.push_back(symbol_stack.back());
            symbol_stack.pop_back();
          }
      ...
      case token::mod:  //%
        if (symbol_stack.empty() ||
            preceden(token::mod) > preceden(std::get<0>(symbol_stack.back())))
          symbol_stack.push_back(scan.now_token);
        else {
          while (preceden(token::mod) <=
                 preceden(std::get<0>(symbol_stack.back()))) {
            res.push_back(symbol_stack.back());
            symbol_stack.pop_back();
          }
...
std::string parser::parser_expression() {
  // TODO 这里还有可能是函数调用
  std::vector<std::tuple<token, std::string>> pos_experssion =
      parser_pre_to_pos();
      ...
      case token::end: {
          ...
          while (symbol_stack.size() != 0) {
            ...
            if (!rax_init) {
              ...
              //是左值
                ...
                // table->asm_code_mov_auto_r__(size, "ax",
                // table->find_local(a));
                rax_init = true;
              }
            }
        ...
        if (num_stack.size() == 1) {
          if (std::get<0>(scan.last_token) == token::identif) {
            //
table->asm_code_mov_auto_r__(size_class.at(std::get<0>(scan.last_token)),
            // "ax", table->find_local(num_stack[0]));
            return;
          }  //是右值
          else {
            //
table->asm_code_mov_auto_r__(size_class.at(std::get<0>(scan.last_token)),
      ...
      case token::plus: {   //+
        if (!symbol_stack.empty() &&
            preceden(token::plus) <= preceden(symbol_stack.back())) {
          while (!symbol_stack.empty() &&
                 preceden(token::plus) <= preceden(symbol_stack.back())) {
            std::string a = num_stack.back();
            num_stack.pop_back();
            ...
            if (!rax_init) {
              //
table->asm_code_mov_auto___r(size_class.at(table->find_var_class(a)),
              // table->find_local(a), "ax");
              rax_init = true;
            }
            num_stack.push_back(b);
          }
      ...
      case token::minus: {  //-
        if (!symbol_stack.empty() &&
            preceden(token::minus) <= preceden(symbol_stack.back())) {
          while (!symbol_stack.empty() &&
                 preceden(token::minus) <= preceden(symbol_stack.back())) {
            std::string a = num_stack.back();
            num_stack.pop_back();
...
table->asm_code_mov_auto___r(size_class.at(table->find_var_class(a)),
            ...
            //
            //
table->asm_code_add_auto_r__(size_class.at(table->find_var_class(b)),
            // "ax", table->find_local(b));
            //要把弹出的结果入栈，但是这里ax已经起了栈顶的作用，所有就假装 b
            //塞一个进去
            num_stack.push_back(b);
          }
        }
  ...
  if (scan.get_current_token() == token::l_mid_par) {
    ...
    switch (class_indentfi) {
      case token::key_int: {
        scan.next_token();
        code.push_back(
            statement(token::dec_int_arrary, name_array, size_array, ""));
      ...
      case token::key_char: {
        scan.next_token();
        code.push_back(
            statement(token::dec_char_arrary, name_array, size_array, ""));
      ...
      case token::key_bool: {
        scan.next_token();
        code.push_back(
            statement(token::dec_bool_arrary, name_array, size_array, ""));
      ...
      case token::key_double: {
        scan.next_token();
        code.push_back(
            statement(token::dec_double_arrary, name_array, size_array, ""));
...
void parser::parser_function_define_or_declare(std::string func_name,
...
ast::idx parser::parser_statement() {
  ...
  switch (scan.get_current_token()) {
    ...
    case token::identif: {
      ...
      switch (scan.get_current_token()) {
        ...
        case token::plus_agn: {  //+=
          ...
          r_value = parser_expression();
          code.push_back(
              statement(token::plus_agn, r_value, "", name_indentif));
          return;
        }
        case token::minus_agn: {  //-=
          scan.next_token();
        ...
        default: {
          ...
          fmt::print("the token is {} line:{} column:{}\n",
                     token_to_string.at(scan.get_current_token()), line,
                     column);
          exit(0);
        }
      }
    ...
    case token::r_char:
    case token::r_int:
    case token::r_double: parser_expression();
...
std::string parser::parser_func_call(std::string name_func) {
  ...
  while (scan.get_current_token() != token::r_par) {
    switch (scan.get_current_token()) {
      ...
      case token::r_int:
        code.push_back(
            statement(token::arg_r_int, scan.get_current_value(), "", ""));
        break;
      case token::r_double:
        code.push_back(
            statement(token::arg_r_double, scan.get_current_value(), "", ""));
        break;
      case token::r_bool:
        code.push_back(
            statement(token::arg_r_bool, scan.get_current_value(), "", ""));
        break;
      case token::r_char:
        code.push_back(
            statement(token::arg_r_char, scan.get_current_value(), "", ""));
        break;
      case token::identif: {
        token class_arg = table->find_var_class(scan.get_current_value());
        switch (class_arg) {
          case token::class_int:
            code.push_back(statement(token::arg_int_class,
                                     scan.get_current_value(), "", ""));
            break;
          case token::class_double:
            code.push_back(statement(token::arg_double_class,
                                     scan.get_current_value(), "", ""));
            break;
          case token::class_char:
            code.push_back(statement(token::arg_char_class,
                                     scan.get_current_value(), "", ""));
            break;
          case token::class_bool:
            code.push_back(statement(token::arg_bool_class,
                                     scan.get_current_value(), "", ""));
            break;
          default:
      ...
      default: {
        fmt::print("\na unknow error at parser::parser_func_call() line:{} "
                   "colume:{}\n",
                   line, column);
        exit(0);
      }
...
std::string parser::get_var_time() {
...
std::string parser::get_label() {
  std::string re = fmt::format("label{}", label);
  ++label;
  return re;
}

void parser::parser_declare_or_function_define_or_declare() {
  ...
  switch (scan.get_current_token()) {
    case token::l_par: {
      // parser_function_define_or_declare(std::get<1>(identifier_name),
      // std::get<0>(identifier_type));
      break;
    }
    // TODO
    case token::l_mid_par: {
    ...
    default: {
      ...
      fmt::print("the token is {}",
                 token_to_string.at(scan.get_current_token()));
      exit(0);
    }
  }
  return;
...
//解析数组元素
std::string parser::parser_unit(std::string name_array) {
  scan.next_token();  //[
...
void parser::print_mid_code() {
  ...
  if (c == 'f') {
    ...
    for (std::size_t i = 0; i < code.size(); ++i) {
      fmt::print("|{:18}|{:18}|{:18}|{:18}|\n",
                 token_to_string.at(code[i].symbol), code[i].arg1, code[i].arg2,
                 code[i].result);
    }
    fmt::print("+------------------+------------------+------------------+-----"
               "-------------+\n");
    ...
    fmt::print("no {} cmd, please enter f(file) or t(terminal)\n", c);
...
std::vector<ast::node>& parser::get_syntax_tree() {
  return tree.get_syntax_tree();
}
...
ast::idx parser::parser_declaration_or_definition() {
  ast::idx idx_root
    = tree.creat_node(ast::node_type::declaration_or_definition);
...
ast::idx parser::parser_declarator() {
  ...
  tree[idx_root].value.declarator.idx_direct_declarator
...
ast::idx parser::parser_identifier() {
  ...
  for (std::size_t i = 0;
       i < std::min(static_cast<std::size_t>(25), str.size()); ++i) {
...
ast::idx parser::parser_assignment_expression(ast::idx last_assign) {
  ...
  switch (scan.get_current_token()) {
    ...
    case token::bit_xor_agn: {
      ast::idx idx_root
        = tree.creat_node(ast::node_type::assignment_expression);
    ...
    case token::question_mark: {
      scan.next_token();
      ast::idx idx_root
        = tree.creat_node(ast::node_type::conditional_expression);
...
ast::idx parser::parser_constant() {
  ...
  scan.next_token();
  for (std::size_t i = 0; i < std::min(str.size(), ast::array_in_struct_size);
       ++i) {
