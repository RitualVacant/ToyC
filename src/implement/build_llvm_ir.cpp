...
// 2.
/**
 * @brief build signal function argument
 *
...
build_llvm_ir::build_argument_declaration(ast::idx idx_argument_declaration)
{
  ...
  // get identifier name
  // for loop will down to the direct_declarator node
  ...
  return {ptr_type_argument, std::string(tree[idx_identifier].value.identifier.name)};
}
...
)
{
  ...
  else if (tree[idx_direct_declarator].value.direct_declarator.idx_arguments_type_list != ast::null)
  {
    ...
    std::tuple<llvm::SmallVector<llvm::Type *>, llvm::SmallVector<std::string>>
      argument_type_list = build_arguments_type_list(
        tree[idx_direct_declarator].value.direct_declarator.idx_arguments_type_list
      );

    llvm::FunctionType *ptr_func_type
      = llvm::FunctionType::get(ptr_return_type, std::get<0>(argument_type_list), false);
    ...
    else
    {
      ...
      {
        unsigned idx = 0;
        for (auto &arg : ptr_func->args())
        {
          std::string name = std::get<1>(argument_type_list)[idx++];
          insert_func_argument(name, &arg, arg.getType());
        }
  ...
  else if (tree[idx_declaration_declarator].value.declaration_declarator.type == ast::declarator_type::type_struct)
  {
    ...
    if (now_compound_statement != ast::null)
    {
      llvm::SmallVector<llvm::Type *> element_type_list
        = build_struct_element_type_list(now_compound_statement);
      llvm::StructType *ptr_struct_type
        = llvm::StructType::create(*context, element_type_list);
      struct_union_table.insert({name, ptr_struct_type});
      return;
    }
    // struct declaration
    else
    {
      llvm::StructType *ptr_struct_type = llvm::StructType::create(*context);
      struct_union_table.insert({name, ptr_struct_type});
      return;
    }
  }
...
llvm::Value *build_llvm_ir::build_postfix_expression(ast::idx idx_postfix_expression)
{
  ...
  if (tree[idx_primary_expression].value.primary_expression.idx_identifier != ast::null)
  {
    ...
    std::string name_identifier = std::string(tree[idx_identifier].value.identifier.name);
        ...
        switch (tree[idx_postfix_operator].value.postfix_operator.postfix_operator)
        {
          ...
          case token::invalid: {
            llvm::Value *value_identifier
              = search_value(tree[idx_identifier].value.identifier.name);
            llvm::Type *type_identifier
              = search_type(tree[idx_identifier].value.identifier.name);

            // CASE array
            if (tree[idx_postfix_operator]
                  .value.postfix_operator.idx_array_idx_assignment_expression)
            {
              llvm::SmallVector<llvm::Value *> idx_list;
              while (idx_postfix_operator != ast::null)
              {
                idx_list.push_back(build_assign_expression(
                  tree[idx_postfix_operator]
                    .value.postfix_operator.idx_array_idx_assignment_expression
                ));
              }
              return builder->CreateGEP(
                type_identifier->getArrayElementType(), value_identifier, idx_list
              );
            }
            // CASE function call
            else if (tree[idx_postfix_operator]
                       .value.postfix_operator.idx_func_call_assignment_expression_list)
            {
              return builder->CreateCall(
                func_table.at(name_identifier),
    ...
    {
      std::string identifier_name = tree[idx_identifier].value.identifier.name;
      return search_value(identifier_name);
    }
  }
  //------------
  // constant
...
build_llvm_ir::build_struct_element_type_list(ast::idx idx_compound_statement)
{
  llvm::SmallVector<llvm::Type *> struct_element_type_list;
  ast::idx idx_block = tree[idx_compound_statement].value.compound_statement.idx_block;
  // block
  for (ast::idx i = idx_block; i != ast::null; i = tree[i].value.block.idx_next_block)
  {
    // declarations
    ast::idx idx_declaration = tree[i].value.block.idx_declaration;
    for (ast::idx j = idx_declaration; j != ast::null;
         j = tree[j].value.declaration_or_definition.idx_next_declaration_or_definition)
    {
      ast::idx idx_declaration_declarator
        = tree[j].value.declaration_or_definition.idx_declaration_declarator;
      for (ast::idx k = tree[j].value.declaration_or_definition.idx_initial_declarator;
           k != ast::null;
           k = tree[k].value.initial_declarator.idx_next_initial_declarator)
      {
        struct_element_type_list.push_back(
          build_struct_element_type(idx_declaration_declarator, k)
        );
      }
    }
  }
  return struct_element_type_list;
}

/**
 * @brief build arguments list when function was called
 *
 * @param idx_assignment_list
 * @return llvm::SmallVector<llvm::Value *>
 */
llvm::SmallVector<llvm::Value *>
build_llvm_ir::build_arguments_list(ast::idx idx_assignment_list_node)
{
  llvm::SmallVector<llvm::Value *> arguments_list;
  for (ast::idx i = idx_assignment_list_node; i != ast::null;
       i          = tree[i].value.arguments_list_node.idx_next_arguments_list_node)
  {
    arguments_list.push_back(
      build_assign_expression(tree[i].value.arguments_list_node.idx_assignment_expression)
    );
  }
 ...
 * @return llvm::Value*
 */
llvm::Type *build_llvm_ir::build_struct_element_type(
  ast::idx idx_declaration_declarator,
  ast::idx idx_initial_declarator
)
{
  ...
  {
    ast::idx idx_struct_type_identifier
      = tree[idx_declaration_declarator]
          .value.declaration_declarator.idx_struct_union_identifier;
    return struct_union_table.at(
      std::string(tree[idx_struct_type_identifier].value.identifier.name)
    );
  }
  // ----------------------------------
  // var
 ...
 * @param ptr_type
 */
void build_llvm_ir::insert_func_argument(
  std::string  name,
  llvm::Value *ptr_var,
  llvm::Type  *ptr_type
)
{
  if (func_argument_table.find(name) == func_argument_table.end())
  {
    func_argument_table.insert({
      name, {ptr_var, ptr_type}
    });
  }
  else
  {
    fmt::print(fg(fmt::color::red), "this argument name already exists");
    exit(0);
  }
}
  ...
  else if (tree[idx_binary_or_unary_expression].type == ast::node_type::unary_expression)
  {
    ...
    else if (tree[idx_binary_or_unary_expression].value.unary_expression.idx_postfix_expression != ast::null)
    {
      ...
      if (tree[idx_primary_expression].value.primary_expression.idx_identifier != ast::null)
      {
        ast::idx idx_identifier
          = tree[idx_primary_expression].value.primary_expression.idx_identifier;
        // postfix operator not exist,
        if (tree[idx_postfix_expression].value.postfix_expression.idx_postfix_operator == ast::null)
        {
          return search_type(std::string(tree[idx_identifier].value.identifier.name));
        }
        // postfix operator exist,
        else
        {
          std::string name_identifier
            = std::string(tree[idx_identifier].value.identifier.name);
          ast::idx idx_postfix_operator
            = tree[idx_postfix_expression].value.postfix_expression.idx_postfix_operator;
          // array
          if (tree[idx_postfix_operator].value.postfix_operator.idx_array_idx_assignment_expression != ast::null)
          {
            llvm::Type *type_identifier
              = find_type(tree[idx_postfix_expression].value.identifier.name);
            tree[idx_postfix_expression].llvm_type
              = type_identifier->getArrayElementType();
            return tree[idx_postfix_expression].llvm_type;
          }
          // function call
          else if (tree[idx_postfix_operator]
                     .value.postfix_operator.idx_func_call_assignment_expression_list)
          {
            tree[idx_postfix_expression].llvm_type
              = func_table.at(name_identifier)->getReturnType();
            return tree[idx_postfix_expression].llvm_type;
          }
          // struct number .
          else if (tree[idx_postfix_operator].value.postfix_operator.postfix_operator == token::period)
          {
            TODO
          }
          // pointer number ->
          else if (tree[idx_postfix_operator].value.postfix_operator.postfix_operator == token::ver)
          {
            TODO
          }
          // ++ --
          else if (tree[idx_postfix_operator].value.postfix_operator.postfix_operator == token::self_minus || tree[idx_postfix_operator].value.postfix_operator.postfix_operator
             == token::self_plus)
          {
            return find_type(name_identifier);
          }
          else
          {
            NOT_REACHABLE
...
/**
 * @brief return a value that has been loaded rather than a pointer
 *
 * @param name
 * @return llvm::Value*
 */
llvm::Value *build_llvm_ir::search_value(std::string name)
{
  // function variable
  if (func_symbol_table.find(name) != func_symbol_table.end())
  {
    auto value_type = func_symbol_table.at(name);
    return builder->CreateLoad(std::get<1>(value_type), std::get<0>(value_type));
  }
  // function arguments
  else if (func_argument_table.find(name) != func_argument_table.end())
  {
    return std::get<0>(func_argument_table.at(name));
  }
  // global variables
  ...
  {
    fmt::print(fg(fmt::color::red), "identifier: {} has not been defined\n");
    exit(0);
  }
}
 ...
 * @return llvm::Type*
 */
llvm::Type *build_llvm_ir::search_type(std::string name)
{
  // function variable
  if (func_symbol_table.find(name) != func_symbol_table.end())
  {
  ...
  {
    return std::get<1>(func_argument_table.at(name));
  }
  // global variables
  else if (global_symbol_table.find(name) != global_symbol_table.end())
  {
